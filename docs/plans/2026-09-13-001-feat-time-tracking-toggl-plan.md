...
- **Same history window as cash.** A full sync walks the last three months month by month through `/me/time_entries?start_date&end_date&meta=true` and pulls everything older from the Reports API v3 search endpoint (the v9 endpoint rejects `start_date` earlier than three months ago, seen live on the first production sync; the Reports API has a small hourly quota, so that pull is one range paginated by `first_row_number`). Report-sourced entries carry tag ids but no tag names. Incremental syncs use `since` (which also returns deletions, stored as `deleted_at`). A cursor older than 60 days falls back to a full walk because Toggl's `since` window is short.
